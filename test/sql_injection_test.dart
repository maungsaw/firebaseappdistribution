...
void main() {
  ...
  group('SQL injection resistance', () {
    ...
    test(
      'parameterized insert treats injection payload as literal value',
      () async {
    ...
    test('parameterized delete does not execute injected id clause', () async {
      ...
      final deleted = await db.rawDelete('DELETE FROM tblPolicy WHERE id = ?', [
        maliciousId,
      ]);
    ...
    test(
      'vulnerable string concatenation would be unsafe (reference only)',
      () async {
        final db = await openDatabase(
          inMemoryDatabasePath,
          version: 1,
        ...
        final safe = await db.rawQuery('SELECT * FROM tblUser WHERE name = ?', [
          injection,
        ]);
        expect(safe, isEmpty);
